...
void ClapTrap::takeDamage(unsigned int amount)
{
    if (_hitPoints - amount < 0)
        amount = _hitPoints;
    _hitPoints -= amount;
    std::cout << "ClapTrap " << getName() << " takes " << amount << " damage and now has now " << getHitPoints() << " hitpoints!" << std::endl;
}
...
void ClapTrap::beRepaired(unsigned int amount)
{
    ...
    _hitPoints += amount;
    std::cout << "ClapTrap " << getName() << " uses " << amount << " energy points to repair and now has " << getHitPoints() << " hitpoints!" << std::endl;
}
